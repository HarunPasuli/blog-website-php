...
if ($category == 'all') {
    $query = $connect->prepare("SELECT * FROM products WHERE PID < ? AND (fileName LIKE CONCAT('%',?,'%') OR fileDescription LIKE CONCAT('%',?,'%')) ORDER BY PID DESC LIMIT ?");
    $query->bind_param("issi", $lastId, $search_query, $search_query, $limit);
} else {
    $query = $connect->prepare("SELECT * FROM products WHERE PID < ? AND fileCategories = ? AND (fileName LIKE CONCAT('%',?,'%') OR fileDescription LIKE CONCAT('%',?,'%')) ORDER BY PID DESC LIMIT ?");
    $query->bind_param("issi", $lastId, $category, $search_query, $search_query, $limit);
}
...
if($result->num_rows > 0) {
    ...
    if($result->num_rows < $limit) {
        ...
        <div class="show_more_btn">
            <button type="button" id="'.$lastId.'" data-search-query=" '.$search_query.'" class="btn btn-primary">Load More</button>
        </div>';
    }
} 
else {
