...
<head>
    ...
    <link rel="icon" type="image/x-icon" href="../images/favicon2.png">
    <?php
        // Getting the search query from the URL parameters
        $search_query = $_GET['search'];

        echo "<title>$search_query - Blog Website</title>";
    ?>
...
<?php
    ...
    $sql = "SELECT * FROM products WHERE fileName LIKE '%$search%' OR fileDescription LIKE '%$search%' ORDER BY PID DESC LIMIT 0,9";
    $result = mysqli_query($connect, $sql);
    $queryResult = mysqli_num_rows($result);

    if ($queryResult > 0) {
        $products = array(); // initialize an empty array to store the retrieved products
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $products[] = array( // add the retrieved product to the array
                'fileId' => $fileId,
                'fileName' => $fileName,
                'fileUpload' => $fileUpload,
                'description' => $description,
                'fileProfilePicture' => $fileProfilePicture,
                'filePersonalName' => $filePersonalName,
                'fileProductDate' => $fileProductDate,
                'fileCategories' => $fileCategories
            );
        }
?>
<section id="post-list" class="post container">
  <?php foreach ($products as $product) { // loop through the array of products ?>
  <div class="post-box" data-category="<?php echo $product['fileCategories'] ?>">
  <a href="<?php echo '../html/post-page.php?id='.$fileId.''?>">
    <img src="../AdminDashboard/admin/file/<?php echo $product['fileUpload'];?>" alt="" class="post-img">
  </a>
    <h2 class="category">
      <?php echo $product['fileCategories'] ?>
    </h2>
    <a href="<?php echo '../html/post-page.php?id=' . $product['fileId'];?>" class="post-title">
      <?php echo $product['fileName']; ?>
    </a>
    <span class="post-date" id="creation-date">
      <?php echo $product['fileProductDate'] ?>
    </span>
    <p class="post-description">
      <?php echo $product['description']; ?>
    </p>
    <div class="profile">
      <img src="../AdminDashboard/admin/file/<?php echo $product['fileProfilePicture'];?>" alt="" class="profile-img">
      <span class="profile-name"> <?php echo $product['filePersonalName'] ?> </span>
    </div>
  </div>
  <?php } ?>
</section>
<?php 
    ...
    echo "<p style = 'text-align: center; margin: 3rem 3rem; font-size: 1.5rem;'>No results found.</p>";
  }
?>  

<?php if($queryResult >= 9) {?>
<div class="show_more_btn_container">
  <div class="show_more_btn">
    <button type="button" id="<?php echo $fileId; ?>" data-search="<?php echo $search; ?>" class="btn btn-primary">Load More</button>
  </div>
</div>
<?php } ?>
...
$(document).on("click", ".btn-primary", function() {
  ...
  var search = $(this).data('search'); // retrieve search value from data attribute

  // $('.btn-primary').html("<img src='ajax-loader.gif' height='25px' width='25px'>");
  $(".show_more_btn_container").remove();
  $.ajax({
    url: "../html/search_load_more.php",
    method: "POST",
    data: {
      PID: lastId,
      search: search 
